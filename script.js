...
// Session interactions (once per session)
...
// ==================== FLYER IMAGE (16:9 for card cover) ====================
const FLYER_IMAGE = {
    width: 800,
    height: 450,
    maxFileMB: 5,
...
// ==================== TRANSLATED ICON LABELS ====================
const CONTINENT_NAMES = {
    'africa': 'Afurika',
    'asia': 'Aziya',
    'europe': 'Uburayi',
    'north-america': 'Amerika ya Ruguru',
    'south-america': 'Amerika y\'Epfo',
    'australia': 'Ositariya',
...
// ==================== WORKING RAPIDAPI TRANSLATION FUNCTION ====================
async function translateWithRapidAPILang(text, from = 'en', to = 'rw') {
    ...
    try {
        ...
        if (data && data.translation) {
            console.log('✅ RapidAPI Translation Success:', data.translation);
...
async function translateWithRapidAPI(text) {
    return translateWithRapidAPILang(text, 'en', 'rw');
}
...
async function translateToKinyarwanda(text) {
    ...
    console.log('RapidAPI failed, falling back to MyMemory...');
    return translateWithMyMemoryFallback(text);
}

// MyMemory fallback function
...
async function translateLongText(text) {
    if (text.length < 500) {
        return await translateToKinyarwanda(text);
    }
...
// Generate sequential number for flyer reference
...
// ==================== PICTURE UPLOAD ====================
function clearStoryImages() {
    storyImageOverrides = {};
}

function getStoryImageForIndex(storyIndex) {
    const story = extractedStories[storyIndex];
    if (!story) return null;
    if (storyImageOverrides[storyIndex]) return storyImageOverrides[storyIndex];
    if (story.image) return normalizeFlyerImage(story.image, story);
    return getImageForStory(story);
}

function setStoryImage(storyIndex, imageSrc) {
    if (!extractedStories[storyIndex]) return;
    storyImageOverrides[storyIndex] = imageSrc;
    extractedStories[storyIndex].image = imageSrc;
    updateStoryImagePreview(storyIndex);
}
...
function resizeImageForFlyer(file) {
    return new Promise((resolve, reject) => {
        ...
        if (file.size > FLYER_IMAGE.maxFileMB * 1024 * 1024) {
        ...
        reader.onload = (e) => {
            const img = new Image();
            img.onload = () => {
                const tw = FLYER_IMAGE.width;
                const th = FLYER_IMAGE.height;
...
async function uploadFlyerPictureForStory(event, storyIndex) {
    ...
    try {
        ...
        else if (err.message === 'invalid') alert(`⚠️ ${FLYER_PHOTO_INVALID}`);
        else alert(`⚠️ ${FLYER_PHOTO_INVALID}`);
    }
    event.target.value = '';
}

function syncStoryImageFromUrlInput(storyIndex) {
    const current = getStoryImageForIndex(storyIndex);
    if (isDataImageUrl(current)) return;

    const input = document.getElementById(`flyer-image-url-${storyIndex}`);
    if (!input || !input.value.trim()) return;
    const parsed = parseFlyerImageUrlInput(input.value);
    if (!parsed) return;
    const optimized = optimizeRemoteFlyerUrl(parsed);
    setStoryImage(storyIndex, normalizeFlyerImage(optimized, extractedStories[storyIndex] || {}));
}

async function applyFlyerImageUrl(storyIndex) {
    const input = document.getElementById(`flyer-image-url-${storyIndex}`);
    if (!input) return;

    const parsed = parseFlyerImageUrlInput(input.value);
    if (!parsed) {
        alert(`⚠️ ${FLYER_PHOTO_URL_INVALID}`);
        return;
    }

    const optimized = optimizeRemoteFlyerUrl(parsed);
    const normalized = normalizeFlyerImage(optimized, extractedStories[storyIndex] || {});

    try {
        await preloadImageUrl(normalized);
    } catch {
        const useAnyway = confirm(`⚠️ ${FLYER_PHOTO_URL_FAILED}\n\nGusa ukomeze ukoreshe iyi link?`);
        if (!useAnyway) return;
    }

    setStoryImage(storyIndex, normalized);
    input.value = normalized;
    alert(`✅ ${FLYER_PHOTO_SAVED}`);
}

function updateStoryImagePreview(storyIndex) {
    const el = document.getElementById(`story-image-preview-${storyIndex}`);
    if (!el) return;
    const src = getStoryImageForIndex(storyIndex);
...
function renderFlyerImageControls(storyIndex) {
    const src = getStoryImageForIndex(storyIndex) || '';
    ...
    return `
        <div class="flyer-image-picker">
            ...
            <div class="flyer-image-picker-preview ${hasClass}" id="story-image-preview-${storyIndex}" role="img" aria-label="Preview">
                <img class="flyer-image-picker-img" alt="">
            </div>
            <div class="flyer-image-picker-actions">
                <label class="flyer-image-file-btn">
                    📷 ${FLYER_PHOTO_PICK}
                    <input type="file" accept="image/*" onchange="uploadFlyerPictureForStory(event, ${storyIndex})">
                </label>
                <div class="flyer-image-url-row">
                    <input type="url" id="flyer-image-url-${storyIndex}" placeholder="${FLYER_PHOTO_URL_PLACEHOLDER}" value="${urlValue}">
                    <button type="button" class="flyer-image-url-btn" onclick="applyFlyerImageUrl(${storyIndex})">${FLYER_PHOTO_APPLY}</button>
                </div>
            </div>
...
function parseFileContent(content) {
    const stories = [];
    const flyerBlocks = content.split('---').filter(block => block.trim().length > 0);
...
// ==================== STORY EXTRACTION WITH TRANSLATION ====================
async function extractStories() {
    ...
    try {
        ...
        if (stories.length === 0) {
            ...
            alert(`✅ Inkuru ${stories.length} ${TRANSLATION_SUCCESS}`);
        }
    } catch (error) {
...
function extractKeywords(text) {
    const commonWords = ['na', 'ya', 'mu', 'ku', 'ni', 'uko', 'kuko', 'cya', 'rya', 'muri', 'the', 'a', 'an', 'and', 'or'];
    ...
    return Object.entries(wordFreq).sort((a, b) => b[1] - a[1]).slice(0, 5).map(entry => entry[0]);
}

function extractDate(text) {
    const yearMatch = text.match(/\b(17|18|19|20)\d{2}\b/);
...
function extractRegion(text) {
    ...
    for (const [region, keywords] of Object.entries(regions)) {
        if (keywords.some(k => lowerText.includes(k))) return region;
    }
    return 'Afurika';
}
...
function detectContinent(text) {
    ...
    for (const [continent, keywords] of Object.entries(continentKeywords)) {
        if (keywords.some(k => lowerText.includes(k))) return continent;
    }
    return 'worldwide';
}
...
function detectCategory(text) {
    const lowerText = text.toLowerCase();
    if (lowerText.includes('ikirere') || lowerText.includes('kwezi') || lowerText.includes('space')) return 'cosmic';
    if (lowerText.includes('afurika') || lowerText.includes('umugabane')) return 'continental';
    return 'worldwide';
}
...
function displayStoryPreviews(stories) {
    ...
    previewDiv.style.display = 'block';
    stories.forEach((_, index) => updateStoryImagePreview(index));
}
...
function generateFlyer(storyIndex) {
    ...
    if (isDuplicateFlyer(story.title, story.fullDescription)) {
        alert(`⚠️ ${DUPLICATE_WARNING}!`);
        return;
    }
...
function closeEditor() {
    ...
    if (savedIndex != null && extractedStories.length) {
        displayStoryPreviews(extractedStories);
    }
}
...
function showTab(tabName) {
    ...
    if (generatorTab) generatorTab.style.display = tabName === 'generator' ? 'block' : 'none';
    if (collectionTab) collectionTab.style.display = tabName === 'collection' ? 'block' : 'none';
    ...
    if (tabCollection) tabCollection.classList.toggle('active', tabName === 'collection');
...
function updateCollectionCount() {
    ...
    if (countSpan) countSpan.textContent = flyersCollection.length;
}
...
function filterByContinent(continent) {
    ...
    displayFlyers();
    updateActiveFilters();
    // Update the quote roll to reflect the selected continent
    if (typeof renderQuoteRoll === 'function') {
        renderQuoteRoll(continent);
    }
}
...
function sortBy(sortType) {
    ...
    const sortSpan = document.getElementById('selectedSort');
    if (sortSpan) sortSpan.textContent = names[sortType] || 'Ibiheruka';
    displayFlyers();
}
...
function isDuplicateFlyer(title, description) {
    const normTitle = title.toLowerCase().trim();
    return flyersCollection.some(f => f.title.toLowerCase().trim().includes(normTitle));
}
...
function toggleFlyerExpansion(id, e) {
    ...
    if (expandedFlyers.has(idKey)) {
        expandedFlyers.delete(idKey);
    } else {
        expandedFlyers.add(idKey);
    }
    displayFlyers();
}
...
function handleLike(id, e) {
    ...
    const flyer = flyersCollection.find(f => flyerIdKey(f.id) === idKey);
    if (flyer) {
...
function toggleComments(id, e) {
    e.stopPropagation();
    const idKey = flyerIdKey(id);
    expandedComments.has(idKey) ? expandedComments.delete(idKey) : expandedComments.add(idKey);
    displayFlyers();
}
...
function addComment(id, e) {
    ...
    if (!input) {
        console.log('Comment input not found for id:', id);
        return;
    }
    const text = input.value.trim();
    if (text) {
        const idStr = String(id);
        const flyer = flyersCollection.find(f => String(f.id) === idStr);
        if (flyer) {
...
function displayFlyers() {
    ...
    grid.innerHTML = filtered.map((f, idx) => createFlyerCard(f, idx)).join('');
}

function createFlyerCard(f, displayIndex) {
    const idKey = flyerIdKey(f.id);
    const exp = expandedFlyers.has(idKey);
    const comExp = expandedComments.has(idKey);
    const liked = sessionInteractions.liked.has(idKey);
    const knew = sessionInteractions.knew.has(idKey);
    const didnt = sessionInteractions.didntKnow.has(idKey);
        ...
        <div class="flyer-card ${exp ? 'expanded' : ''} ${styleClass} ${f.colorClass}" onclick="toggleFlyerExpansion('${f.id}', event)" style="position: relative;">
            ...
            <div class="flyer-content" style="padding: 12px;">
                <h3 class="flyer-title" style="margin-bottom: 6px; font-size: 0.95rem; padding-right: 25px;">
                    ${f.title}
                    <span class="translation-badge" style="background: #3498db; color: white; padding: 2px 6px; border-radius: 12px; font-size: 0.55rem; margin-left: 6px;">
                        <i>🇷🇼</i> Kinyarwanda
                    </span>
            ...
            `<div class="flyer-description-preview" style="margin-bottom: 10px; font-size: 0.8rem;">${f.previewDescription || (f.fullDescription ? f.fullDescription.substring(0, 100) + '...' : 'Nta bisobanuro')}<div class="read-more-hint" style="color: #999; font-size: 0.6rem; margin-top: 3px;">👇 ${CLICK_TO_READ}</div></div>`
                ...
                <div class="interaction-bar" style="display: flex; gap: 5px; flex-wrap: wrap;">
                    <button class="interaction-btn ${liked ? 'liked' : ''}" onclick="handleLike('${f.id}', event)" ${liked ? 'disabled' : ''} style="background: ${liked ? '#e74c3c' : '#34495e'}; color: white; border: none; padding: 5px 10px; border-radius: 20px; cursor: pointer; display: inline-flex; align-items: center; gap: 3px; font-size: 0.7rem;">
                        ❤️ ${f.likes || 0}
                    </button>
                    <button class="interaction-btn" onclick="toggleComments('${f.id}', event)" style="background: #34495e; color: white; border: none; padding: 5px 10px; border-radius: 20px; cursor: pointer; display: inline-flex; align-items: center; gap: 3px; font-size: 0.7rem;">
                        💬 ${f.comments?.length || 0}
                    </button>
                    <button class="interaction-btn ${knew ? 'knew-active' : ''}" onclick="handleKnew('${f.id}', event)" ${knew ? 'disabled' : ''} style="background: ${knew ? '#2ecc71' : '#34495e'}; color: white; border: none; padding: 5px 10px; border-radius: 20px; cursor: pointer; display: inline-flex; align-items: center; gap: 3px; font-size: 0.7rem;">
                        ✅ ${f.knew || 0}
                    </button>
                    <button class="interaction-btn ${didnt ? 'didntknow-active' : ''}" onclick="handleDidntKnow('${f.id}', event)" ${didnt ? 'disabled' : ''} style="background: ${didnt ? '#e67e22' : '#34495e'}; color: white; border: none; padding: 5px 10px; border-radius: 20px; cursor: pointer; display: inline-flex; align-items: center; gap: 3px; font-size: 0.7rem;">
                        ❓ ${f.didntKnow || 0}
                    </button>
                </div>
                ${comExp ? `
...
function showLoginForm() {
    ...
    if (modalTitle) modalTitle.innerHTML = '<i>🔐</i> Injira';
...
function showRegisterForm() {
    ...
    const loginForm = document.getElementById('loginForm');
    const registerForm = document.getElementById('registerForm');
    if (modalTitle) modalTitle.innerHTML = '<i>📝</i> Iyandikishe';
...
async function handleLogin() {
    ...
    if (!email || !password) {
        alert("Andika imeyili n'ijambo ry'ibanga byawe!");
        return;
    }
    ...
    if (user) {
        ...
        hideLoginModal();
        updateUserStatus();
        alert('✅ Winjiye neza! Murakaza neza ' + (user.name || user.email));
...
async function handleRegister() {
    ...
    if (password.length < 6) {
        alert('Ijambo ry\'ibanga rigomba kugira byibura inyuguti 6');
        return;
    }
    ...
    if (users.find(u => u.email === email)) {
        alert('❌ Iyi imeyili isanzwe ikoreshwa');
        return;
    }
    ...
    hideLoginModal();
    updateUserStatus();
    alert('✅ Iyandikishya ryagenze neza! Murakaza neza ' + name);
    window.location.reload();
}
...
function getCurrentUser() {
    ...
    const userId = localStorage.getItem('wrzkk_user_id');
    const userEmail = localStorage.getItem('wrzkk_user_email');
    const userName = localStorage.getItem('wrzkk_user_name');
    
    if (userRole === 'registered' && userId) {
        return { id: userId, role: 'registered', email: userEmail, name: userName };
    }
    return { id: userId || 'anonymous_' + Date.now(), role: 'anonymous', name: 'Uwihitira' };
}
...
function updateUserStatus() {
    ...
    const numerologyNav = document.getElementById('numerologyNavBtn');
    
    if (userRole === 'registered' && userEmail) {
        if (badge) { badge.className = 'user-badge registered'; badge.innerHTML = `📧 ${userName || userEmail.split('@')[0]}`; }
        if (authButton) { authButton.innerHTML = '👤 Konti yanjye'; authButton.onclick = showUserAccount; }
        if (numerologyNav) numerologyNav.style.display = 'inline-flex';
    } else {
        ...
        if (numerologyNav) numerologyNav.style.display = 'none';
    }
}
...
function showUserAccount() {
    ...
    if (confirm('🐘 Ushaka gusohoka?')) {
        localStorage.removeItem('wrzkk_session');
        localStorage.removeItem('wrzkk_user_id');
        localStorage.removeItem('wrzkk_user_role');
        localStorage.removeItem('wrzkk_user_email');
        localStorage.removeItem('wrzkk_user_name');
        continueAsAnonymous();
        window.location.reload();
    }
}
...
function addColorRefreshButton() {
    ...
    refreshBtn.id = 'colorRefreshBtn';
    refreshBtn.className = 'clear-filter';
    refreshBtn.innerHTML = '<i>🎨</i> Hindura Amabara';
    refreshBtn.onclick = refreshAllColors;
    refreshBtn.style.background = '#9b59b6';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Render the quote roll once on page load
    renderQuoteRoll('worldwide');
});

// ==================== Rwandan Proverb Widget ====================
// ==================== RWANDAN PROVERB DATABASE (LOCAL FALLBACK) ====================
const rwandanProverbs = [
    {
      "kinyarwanda": "Ababiri bagiye inama baruta umunani barasana.",
      "translation": "Two who consult are better than eight who clash.",
      "lesson": "Consultation leads to harmony; lack of it leads to conflict."
    },
    {
      "kinyarwanda": "Ababiri bakika umwe.",
      "translation": "Two break one.",
      "lesson": "Unity or combined effort can overcome a single force."
    },
    {
      "kinyarwanda": "Ababiri bateranya abeza.",
      "translation": "Two gather the good ones.",
      "lesson": "Collaboration helps select what is valuable."
    },
    {
      "kinyarwanda": "Ababiri bica umwe.",
      "translation": "Two kill one.",
      "lesson": "Collective action can overpower an individual."
    },
    {
      "kinyarwanda": "Ababiri ntibacibwa inka.",
      "translation": "Two are not deprived of a cow.",
      "lesson": "Unity prevents dispossession."
    },
    {
      "kinyarwanda": "Abacuranye ubusa basangira ubundi.",
      "translation": "Those who trade in vain share another (trade).",
      "lesson": "Losses in one venture can be compensated by sharing in another."
    },
    {
      "kinyarwanda": "Abadapfuye ntibabura kubonana.",
      "translation": "The living never fail to meet again.",
      "lesson": "Life brings people together repeatedly."
    },
    {
      "kinyarwanda": "Abagabanye imbisi ntibagabana umufa.",
      "translation": "Those who share raw meat do not share the broth.",
      "lesson": "Sharing the primary gain does not guarantee sharing the secondary benefits."
    },
    {
      "kinyarwanda": "Abagabo babiri ntibabana mu nzu imwe.",
      "translation": "Two men do not live in one house.",
      "lesson": "Too much male authority in one household breeds conflict."
    },
    {
      "kinyarwanda": "Abagabo bararya imbwa zikishyura (zikaryora).",
      "translation": "Men eat dogs that lick themselves.",
      "lesson": "People benefit from those who serve or flatter them."
    },
    {
      "kinyarwanda": "Abagira amenyo baraseka.",
      "translation": "Those who have teeth laugh.",
      "lesson": "Those with means or power enjoy life."
    },
    {
      "kinyarwanda": "Abagira impingo ibigega biragwa.",
      "translation": "Those who have backbones, their granaries fall.",
      "lesson": "The strong or proud may still face ruin."
    },
    {
      "kinyarwanda": "Abagira inyonjo bagira ibirori.",
      "translation": "Those who have cleanliness have feasts.",
      "lesson": "Cleanliness attracts abundance and celebration."
    },
    {
      "kinyarwanda": "Abagiye inama Imana irabasanga.",
      "translation": "Those who go to counsel, God finds them.",
      "lesson": "God blesses those who seek advice."
    },
    {
      "kinyarwanda": "Abahiga ubugabo baratabarana.",
      "translation": "Those who hunt for manhood help each other.",
      "lesson": "Those striving for maturity or courage support one another."
    },
    {
      "kinyarwanda": "Abahigi benshi bayobya imbwa uburari.",
      "translation": "Many hunters mislead the dog into the thicket.",
      "lesson": "Too many leaders cause confusion."
    },
    {
      "kinyarwanda": "Abahiniye hamwe bahima (bahenda) abari hanze.",
      "translation": "Those who bend together drive away outsiders.",
      "lesson": "United insiders exclude outsiders."
    },
    {
      "kinyarwanda": "Abahizi babiri ntibanyura inzira imwe.",
      "translation": "Two adulterers do not pass the same road.",
      "lesson": "Accomplices in wrongdoing avoid being seen together."
    },
    {
      "kinyarwanda": "Abajyanama babiri bishe umujyanama umwe.",
      "translation": "Two advisors killed one advisor.",
      "lesson": "A divided council can destroy a leader."
    },
    {
      "kinyarwanda": "Abajyanama bacishije ukubiri ibyabo ntibishyika.",
      "translation": "Advisors who hide their double intentions do not succeed.",
      "lesson": "Duplicity in counsel leads to failure."
    },
    {
      "kinyarwanda": "Abajyanama bishe imbwa y'umwami irahera.",
      "translation": "Advisors killed the king's dog, it rots.",
      "lesson": "Destroying a loyal servant of authority brings decay."
    },
    {
      "kinyarwanda": "Abajya inama ari babiri banyaga inka z'abarasa munani.",
      "translation": "Two who go to counsel stole the cows of eight strong men.",
      "lesson": "Wise planning can overcome greater numbers."
    },
    {
      "kinyarwanda": "Abajya impaka ari babiri umwe aba yigiza nkana.",
      "translation": "Two who go to dispute, one becomes stubborn.",
      "lesson": "In arguments, one party often becomes obstinate."
    },
    {
      "kinyarwanda": "Abakingiranye inyegamo ntibakingirana ingabo.",
      "translation": "Those who shield each other with a branch do not shield each other with shields.",
      "lesson": "Small gestures of help do not equal full protection."
    },
    {
      "kinyarwanda": "Abakiranye ntibahishana ibibuno.",
      "translation": "Those who have reconciled do not hide their buttocks from each other.",
      "lesson": "True reconciliation brings complete openness."
    },
    {
      "kinyarwanda": "Abakobwa ni nyampinga.",
      "translation": "Girls are beauties.",
      "lesson": "Daughters are precious and valued."
    },
    {
      "kinyarwanda": "Abakundanye barajyana.",
      "translation": "Lovers accompany each other.",
      "lesson": "Love fosters companionship."
    },
    {
      "kinyarwanda": "Abakundanye ni bo bangana.",
      "translation": "Those who love each other are equals.",
      "lesson": "True love creates equality."
    },
    {
      "kinyarwanda": "Abakunzi baza bishize.",
      "translation": "Lovers come when it is late.",
      "lesson": "Lovers arrive after the fact or belatedly."
    },
    {
      "kinyarwanda": "Abakwe ni aba mbere; ibya nyuma bica amazuru.",
      "translation": "Suitors come first; later events cut the noses.",
      "lesson": "Early advantages may later become liabilities."
    },
    {
      "kinyarwanda": "Abana ba Samusure bavukana isunzu.",
      "translation": "Children of Samusure are born with a lock of hair.",
      "lesson": "Some characteristics are inborn."
    },
    {
      "kinyarwanda": "Abana basangira ibere ntibasangira umugisha.",
      "translation": "Children who share the breast do not share the blessing.",
      "lesson": "Early shared resources do not guarantee later shared blessings."
    },
    {
      "kinyarwanda": "Abana ni babiri: Gahanwa na Gahannyi.",
      "translation": "Children are two: the condemned and the condemner.",
      "lesson": "In every situation there is the accused and the accuser."
    },
    {
      "kinyarwanda": "Abana ni batatu: Uwibwira, Ubwirwa na Tereriyo.",
      "translation": "Children are three: the thinker, the told, and Tereriyo.",
      "lesson": "People have different roles: advisor, advised, and the listener."
    },
    {
      "kinyarwanda": "Abahanga babiri ntibotsa igihaha.",
      "translation": "Two experts do not brew sour beer.",
      "lesson": "Too many experts spoil the task."
    },
    {
      "kinyarwanda": "Abangana ubwenge basa ntibabana.",
      "translation": "Those equal in intelligence do not live together.",
      "lesson": "Equals in cleverness clash in close quarters."
    },
    {
      "kinyarwanda": "Abantu n'ibintu ni magirirane.",
      "translation": "People and things are mutual.",
      "lesson": "People and possessions interact and affect each other."
    },
    {
      "kinyarwanda": "Abaryi b'imigongo baribwire.",
      "translation": "Eaters of backs should eat alone.",
      "lesson": "Those who exploit others should not expect company."
    },
    {
      "kinyarwanda": "Abasangira bashonje ntawe usigariza undi.",
      "translation": "Those who share when hungry, none leaves for another.",
      "lesson": "In real need, sharing is immediate and complete."
    },
    {
      "kinyarwanda": "Abasangiye basigana imbyiro.",
      "translation": "Those who shared leave each other the leftovers.",
      "lesson": "Past sharing creates obligations for the future."
    },
    {
      "kinyarwanda": "Abasangira ubusa bitana ibisambo.",
      "translation": "Those who share emptiness call each other names.",
      "lesson": "Sharing poverty leads to insults."
    },
    {
      "kinyarwanda": "Abasangiye imfizi ntibasekana amahembe magufi.",
      "translation": "Those who shared a bull do not laugh at short horns.",
      "lesson": "Those who shared a resource do not mock its minor defects."
    },
    {
      "kinyarwanda": "Abasa barasangira.",
      "translation": "The similar share.",
      "lesson": "Like attracts like and shares together."
    },
    {
      "kinyarwanda": "Abaswa ntiberekwa imishinga.",
      "translation": "The termite-ridden are not shown spears.",
      "lesson": "Don't tempt those already vulnerable."
    },
    {
      "kinyarwanda": "Abatanye badatata barasubirana.",
      "translation": "Those who separated without fighting reconcile.",
      "lesson": "Peaceful separation allows for later reunion."
    },
    {
      "kinyarwanda": "Abateranye imigeri ntibahisha n'amabya.",
      "translation": "Those who share a riverbank do not hide even the small stones.",
      "lesson": "Close neighbors share everything openly."
    },
    {
      "kinyarwanda": "Abaturanye babyarana abana basa.",
      "translation": "Neighbors beget similar children.",
      "lesson": "Neighbors influence each other's offspring."
    },
    {
      "kinyarwanda": "Abatutira batongana batura ukubiri.",
      "translation": "Those who insult each other and then reconcile become twice as close.",
      "lesson": "Reconciliation after conflict strengthens bonds."
    },
    {
      "kinyarwanda": "Abavandimwe iyo bavumbitse akarenge uvumburamo akawe.",
      "translation": "When siblings dig up a foot, you dig up yours.",
      "lesson": "Family members share both labor and discovery."
    },
    {
      "kinyarwanda": "Abegereye uruganda ntibabura urwavumba.",
      "translation": "Those near the forge lack no coal.",
      "lesson": "Proximity to a resource ensures supply."
    },
    {
      "kinyarwanda": "Aberekeranye ntibabura kwendana.",
      "translation": "Those who show each other the way never fail to love each other.",
      "lesson": "Mutual guidance fosters affection."
    },
    {
      "kinyarwanda": "Abeza ba ruguru baseka ibimuga bishinyikiye mu kabande.",
      "translation": "The good from above laugh at the cripples stuck in the ditch.",
      "lesson": "The fortunate mock those trapped in misfortune."
    },
    {
      "kinyarwanda": "Abibeshya b'i Mukarange bagira ngo Nyiraguheka ni nyirasenge.",
      "translation": "The deceivers of Mukarange say Nyiraguheka is the paternal aunt.",
      "lesson": "Lies create false kinship claims."
    },
    {
      "kinyarwanda": "Abishunga b'i Mukarange bagira ngo Nyiraguheka ni nyinawabo.",
      "translation": "Those who hide at Mukarange say Nyiraguheka is their sister.",
      "lesson": "Refuge-seekers claim false kinship."
    },
    {
      "kinyarwanda": "Ab'imbwa bifuza ko budacya.",
      "translation": "Dog owners wish it would not dawn.",
      "lesson": "Those with shameful secrets wish darkness would last."
    },
    {
      "kinyarwanda": "Aboro babiri ntibasangira umwerera.",
      "translation": "Two paupers do not share a single gown.",
      "lesson": "The extremely poor cannot share even minimal resources."
    },
    {
      "kinyarwanda": "Aboro basera guhanya.",
      "translation": "Paupers laugh at division.",
      "lesson": "The destitute mock even the idea of sharing."
    },
    {
      "kinyarwanda": "Abo ntekera impengeri ni bo bantera amabuye.",
      "translation": "Those for whom I weave a basket are the ones who throw stones at me.",
      "lesson": "Those you help may turn against you."
    },
    {
      "kinyarwanda": "Abonye isha itamba ata n'urwo yari yambaye.",
      "translation": "He who found a louse without a head had nothing on.",
      "lesson": "Finding a worthless thing shows one's own poverty."
    },
    {
      "kinyarwanda": "Abotanye kera ntibahishanya amabya.",
      "translation": "Those who interwove long ago do not hide pebbles from each other.",
      "lesson": "Old friends share everything, even small things."
    },
    {
      "kinyarwanda": "Abo umwami yahaye amata ni bo bamwimye amatwi.",
      "translation": "Those to whom the king gave milk are the ones who refused him ears.",
      "lesson": "Those you favor may later ignore you."
    },
    {
      "kinyarwanda": "Abwirwa benshi akumva bene yo.",
      "translation": "Many who are told, only one of them hears.",
      "lesson": "Advice is given to many but heeded by few."
    },
    {
      "kinyarwanda": "Acuritse inkanda ntacuritse umutima.",
      "translation": "He who plaited a headpad did not plait the heart.",
      "lesson": "Appearances do not reflect inner feelings."
    },
    {
      "kinyarwanda": "Agaca amakungu ni ukwima uwarugendagamo.",
      "translation": "Cutting the rope is to miss the one who walked with it.",
      "lesson": "Breaking a bond leaves you longing for the missing partner."
    },
    {
      "kinyarwanda": "Agaciro gake karuta akamaro gacye.",
      "translation": "Small value is better than small usefulness.",
      "lesson": "Intrinsic worth outweighs minor utility."
    },
    {
      "kinyarwanda": "Agacumu gahabwa agahari, naho agahararutswe gahabwa agahini.",
      "translation": "A spear is given to the present, but the one passed over is given to the bent-over.",
      "lesson": "Rewards depend on position and posture."
    },
    {
      "kinyarwanda": "Agacumu kazaguhorera (kazahorera umugabo) ntumenya uwagacuze.",
      "translation": "The spear that will kill you – you don't know who forged it.",
      "lesson": "The cause of your downfall may be unknown."
    },
    {
      "kinyarwanda": "Agahana imbwa ni agashirira.",
      "translation": "The one who feeds dogs is the one who calms them.",
      "lesson": "He who provides controls."
    },
    {
      "kinyarwanda": "Agahanga k'umugabo gahangurwa n'uwakaremye.",
      "translation": "A man's destiny is awakened by his creator.",
      "lesson": "God alone determines one's fate."
    },
    {
      "kinyarwanda": "Agahanga k'umugabo gahuma katavuze.",
      "translation": "A man's destiny becomes silent without speaking.",
      "lesson": "Fate works silently."
    },
    {
      "kinyarwanda": "Agahango gato karuta umugenderano.",
      "translation": "A small promise is better than a long association.",
      "lesson": "A kept small promise outweighs a prolonged but unreliable relationship."
    },
    {
      "kinyarwanda": "Agahararo ntikabuza agahararuko.",
      "translation": "A crowd does not lack a shout.",
      "lesson": "Where there are many people, noise follows."
    },
    {
      "kinyarwanda": "Agaharawe gahabwa agahari, naho agahararutswe gahabwa agahini.",
      "translation": "The one given to is given to the present, the passed over is given to the bent.",
      "lesson": "Rewards go to those present, not to the absent."
    },
    {
      "kinyarwanda": "Agahimbaza umusyi kaba munsi y'ingasire.",
      "translation": "The one who pleases the father-in-law sits under the granary.",
      "lesson": "Honoring elders brings protection and provision."
    },
    {
      "kinyarwanda": "Agahinda gahima indyarya kaba mu rutare kwa Rwirebe.",
      "translation": "Sorrow divides the hypocrite and sits on the rock of Rwirebe.",
      "lesson": "Hypocrites are split by grief."
    },
    {
      "kinyarwanda": "Agahinda gashira akandi kari itumba.",
      "translation": "One sorrow ends, another waits in the dark.",
      "lesson": "Troubles follow one another."
    },
    {
      "kinyarwanda": "Agahinda gashira bake.",
      "translation": "Sorrow ends for few.",
      "lesson": "Few people escape grief."
    },
    {
      "kinyarwanda": "Agahinda k'inkoko kamenya inkike yatoyemo.",
      "translation": "A hen's sorrow is known by the chick that was inside it.",
      "lesson": "A mother's loss is felt by the child she carried."
    },
    {
      "kinyarwanda": "Agahinda k'inkono kamenywa n'uwayiharuye.",
      "translation": "A pot's sorrow is known by the one who scraped it.",
      "lesson": "Only the user knows the wear and tear of a tool."
    },
    {
      "kinyarwanda": "Agahinda ntigashira; gashira nyirako yapfuye.",
      "translation": "Sorrow does not end; it ends when its owner dies.",
      "lesson": "Grief lasts a lifetime."
    },
    {
      "kinyarwanda": "Agahinda ntikajya ahabona.",
      "translation": "Sorrow does not go where it can be seen.",
      "lesson": "People hide their grief."
    },
    {
      "kinyarwanda": "Agahinda ntikica kagira mubi.",
      "translation": "Sorrow does not kill, it makes one evil.",
      "lesson": "Prolonged grief can corrupt character."
    },
    {
      "kinyarwanda": "Agahinda ni ukubura uwo ukunda.",
      "translation": "Sorrow is lacking the one you love.",
      "lesson": "Grief stems from the absence of a loved one."
    },
    {
      "kinyarwanda": "Agahinda si uguhora urira.",
      "translation": "Sorrow is not crying constantly.",
      "lesson": "Grief is internal, not always visible."
    },
    {
      "kinyarwanda": "Agahini gahima indyarya kaba mu rutare rwa Kirebe.",
      "translation": "The bent one divides the hypocrite and sits on Kirebe's rock.",
      "lesson": "Humility exposes hypocrisy."
    },
    {
      "kinyarwanda": "Agahugu Imana yagusasiye ntukarenga.",
      "translation": "The boundary God drew for you, do not cross.",
      "lesson": "Respect divine limits."
    },
    {
      "kinyarwanda": "Agahugu k'abagore ntikabura amazimwe.",
      "translation": "Women's boundary never lacks tears.",
      "lesson": "Women's lives are marked by sorrow."
    },
    {
      "kinyarwanda": "Agahugu karimo indushyi, abapfu ntibabura amazu.",
      "translation": "The boundary has graves, the dead never lack houses.",
      "lesson": "Death claims everyone eventually."
    },
    {
      "kinyarwanda": "Agahuru gakomeye kiyima umupfu.",
      "translation": "A big granary denies itself flour.",
      "lesson": "Great abundance can lead to waste or denial."
    },
    {
      "kinyarwanda": "Agahuru gasabye umuntu inyama ntakarenga.",
      "translation": "A granary that asks a person for meat does not exceed.",
      "lesson": "Resources that demand more than they give are limited."
    },
    {
      "kinyarwanda": "Agahuru kagusabye amaraso ntukarenga.",
      "translation": "A granary that asks you for blood, do not exceed.",
      "lesson": "Don't give more than you can afford to a demanding system."
    },
    {
      "kinyarwanda": "Agahuru gahinyuza inkumi.",
      "translation": "The granary humiliates the unmarried girl.",
      "lesson": "Poverty or dependence shames the young."
    },
    {
      "kinyarwanda": "Agahwa kari ku wundi karahandurika.",
      "translation": "A thorn on another can be removed.",
      "lesson": "It's easier to solve others' problems than your own."
    },
    {
      "kinyarwanda": "Agakambye ugatega u Rwanda (ugatega iminsi).",
      "translation": "The small frog that listens to Rwanda (listens to days).",
      "lesson": "Timing and place are crucial for survival."
    },
    {
      "kinyarwanda": "Agakara gasiga imbwa ntigasiga akako.",
      "translation": "An old hand leaves a dog but does not leave a small hoe.",
      "lesson": "One abandons what is less useful but keeps essential tools."
    },
    {
      "kinyarwanda": "Agakecuru gakize ntikabura abuzukuru.",
      "translation": "A rich old person does not lack grandchildren.",
      "lesson": "Wealth attracts descendants and helpers."
    },
    {
      "kinyarwanda": "Agakecuru gatanze akandi gushoka kagira ngo dore ubwo butama bwako.",
      "translation": "An old person gave another and then said, 'Look at your sheep.'",
      "lesson": "Gifts may come with strings attached."
    },
    {
      "kinyarwanda": "Agakecuru karitse ntikabura abuzukuru.",
      "translation": "An old person who eats alone does not lack grandchildren.",
      "lesson": "Selfish elders still attract heirs due to their wealth."
    }
];
...
async function fetchProverbFromAPI() {
    if (!USE_API) {
        console.log('API disabled, using local database');
        return null;
    }
...
function pickRandomProverbLangs(count = 6) {
    const shuffled = [...PROVERB_LANG_POOL].sort(() => Math.random() - 0.5);
...
async function toggleProverbTranslations(event) {
    ...
    const randomLangs = pickRandomProverbLangs(6);
...
// ==================== QUOTE SCROLL (parchment flyers, subcategory dropdown) ====================
...
// Author pools per subcategory. Add or edit freely — every entry is looked up
// through the same strict match gate, so wrong photos / wrong quotes are
// automatically rejected and the local fallback is used instead.
const QUOTE_AUTHORS_BY_CATEGORY = {
    'africa-indigenous': [
        'Mansa Musa', 'Shaka Zulu', 'Sundiata Keita', 'Queen Nzinga', 'Idris Alooma'
    ],
    'africa-kings': [
        'Haile Selassie', 'Moshoeshoe I', 'Menelik II', 'Cetshwayo', 'Lobengula'
    ],
    'africa-warriors': [
        'Shaka Zulu', 'Queen Amina', 'Samori Ture', 'Taharqa', 'Ahmed Baba'
    ],
    'africa-heroes': [
        'Nelson Mandela', 'Patrice Lumumba', 'Samora Machel', 'Amílcar Cabral', 'Thomas Sankara'
    ],
    'africa-wisemen': [
        'Chinua Achebe', 'Wole Soyinka', 'Ngũgĩ wa Thiong\'o', 'Cheikh Anta Diop', 'Ali Mazrui'
    ],
    'worldwide-famous': [
        'Albert Einstein', 'Mahatma Gandhi', 'Martin Luther King Jr.', 'Winston Churchill', 'Nelson Mandela'
    ],
    'worldwide-continental': [
        'Simón Bolívar', 'José Martí', 'Kwame Nkrumah', 'Jawaharlal Nehru', 'Sun Yat-sen'
    ],
    'worldwide-popular': [
        'Leonardo da Vinci', 'Marie Curie', 'Carl Sagan', 'Stephen Hawking', 'Neil Armstrong'
    ],
    'worldwide-unpopular': [
        'Niccolò Machiavelli', 'Friedrich Nietzsche', 'Karl Marx', 'Ayn Rand', 'Sun Tzu'
    ],
    'worldwide-trending': [
        'Greta Thunberg', 'Malala Yousafzai', 'Yuval Noah Harari', 'Barack Obama', 'Yuval Harari'
    ]
};

// Fallback quotes — used when no online source returns a valid, author-verified quote.
// Keys mirror the subcategory keys above so the fallback always matches the
...
// ============ Author-match helper (used to validate online results) ============
...
// ============ SOURCE 1: Quote Garden (author-filtered) ============
...
// ============ SOURCE 2: Wikiquote (author-page quotes) ============
...
// ============ Master quote fetcher: ONLY author-verified sources ============
...
// ============ SOURCE: Wikipedia portrait (strict last-name match) ============
...
// Handler for the quote-roll's own subcategory <select>.
// NOTE: this does NOT touch `currentFilters` or `filterByContinent()`, so the
// flyers grid, story creation, and the rest of the app are unaffected.
...
// Public entry point. On page load we call renderQuoteRoll() with no argument
// so the default subcategory is used.
...
// Also make sure the dropdown starts on the correct default when the page
// loads (the <option selected> in the HTML handles the visual state, this
// only guarantees the internal state matches).
document.addEventListener('DOMContentLoaded', () => {
    const sel = document.getElementById('quoteContinentSelect');
